...
def extract_dispcurve(frequencies, 
                      ...
                      sta1=None,
                      sta2=None,
                      # --- FastMSPEC instrumentation: new parameter, default preserves upstream
                      # behavior exactly (2-tuple return, plain DispersionCurveException) ---
    ...
    def check_previous_picks(picks,picks_backup,frequency,slope,minvel,maxvel,
        ...
        if len(picks) > 0:
            ...
            # --- FastMSPEC instrumentation: np.in1d -> np.isin (numpy 2.x removed np.in1d;
            # np.isin is the same operation upstream seislib==1.2.1 itself already uses here --
            # this is an environment-compatibility fix, not a behavior change; see NOTES.md) ---
    ...
    def pick_velocity(picks, frequency, densities, slope, x_step, minvel, maxvel,
        ...
        if maxamp > pick_threshold * minamp and maxamp > 0.5*np.max(weights):
            ...
            # maxamp/minamp is exactly the ratio the pick_threshold gate above just tested this
            # pick against -- capture it, since seislib itself discards it after the gate check.
...
#                 fontsize=12,
    ...
    # --- FastMSPEC instrumentation: begin ---
    # `picks` was already converted to an ndarray above (or is `picks_backup`'s own list if that
    # branch was taken); `logspaced_frequencies` is the exact grid the coverage acceptance test
    # (a few lines above) compares `len(smooth_picks)` against -- reuse both rather than
    # recomputing. None of this affects `smooth_picks`/`smooth_picks_x`, which are already final.
    if return_diagnostics:
        _diag_n_accepted_picks = len(picks)
        _diag_freq_coverage_fraction = (
            len(smooth_picks) / len(logspaced_frequencies)
            if len(logspaced_frequencies) > 0 else 0.0
        )
        _diag_mean_amp_ratio = (
            float(np.mean(_diag_amp_ratios)) if len(_diag_amp_ratios) > 0 else float('nan')
        )
        diagnostics = PickDiagnostics(
            converged=len(smooth_picks) > 0,
            bad_quality_fraction=_diag_bad_quality_fraction,
            n_candidate_crossings=_diag_n_candidate_crossings,
    ...
    if return_diagnostics:
        raise DispersionCurveExceptionWithDiagnostics(diagnostics)
    raise DispersionCurveException
