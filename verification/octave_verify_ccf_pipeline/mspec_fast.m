function[varargout]=mspec_fast(FMTSE,varargin)
...
if iscell(x)||iscell(y)
    ...
    varargout=mspec_one(deltat,x,y,psi,lambda,detrendstr,normstr,dim,FMTSE);
end


function[cellout]=mspec_one(dt,x,y,psi,lambda,detrendstr,normstr,dim,FMTSE)
...
[f,mmatx,mmaty,totalMB]=mtrans(x,y,psi,dim); 
...
if isempty(y) %One time series
     if length(lambda)==1
         if isempty(FMTSE)==1
            ...
            cellout{2}=avgspec_sayan(mmatx,mmatx,N,FMTSE,x);
         end
     else
     ...
     if length(lambda)==1
        if isempty(FMTSE)==1
            ...
            cellout{2}=avgspec_sayan(mmatx,mmatx,N,FMTSE,x);
            cellout{3}=avgspec_sayan(mmaty,mmaty,N,FMTSE,y);
            cellout{4}=avgspec_xy_sayan(mmatx,mmaty,FMTSE,x,y,N);
        end
...
function[S]=avgspec(mmat1,mmat2,N)
    eigspec=mmat1.*conj(mmat2); %taking magnitude's square which is power
    S=mean(eigspec,N);


function[S] = avgspec_sayan(mmat1,mmat2,N,FMTSE,x)
    M = FMTSE.N; 
    ...
    z0 = z0(1:floor(FMTSE.N/2)+1,:); %sayan added floor

    eigspec = mmat1.*conj(mmat2); %taking magnitude's square which is power
    if ndims(eigspec)>2
        z1 = sum(eigspec(:,:,FMTSE.index_plus),N)-sum(eigspec(:,:,~FMTSE.index_plus),N);
    else
        z1 = sum(eigspec(:,FMTSE.index_plus),N)-sum(eigspec(:,~FMTSE.index_plus),N);
    end
...
function[S] = avgspec_xy_sayan(mmat1,mmat2,FMTSE,x,y,N)
    M = FMTSE.N; 
    L = M*ceil(2*FMTSE.N/M);
    vecSinc = [2*FMTSE.W;FMTSE.vecHalfSinc;zeros(L-2*FMTSE.N+1,1);flipud(FMTSE.vecHalfSinc)];
    Fx = fft(x,L,1);
    ...
    z0 = ifft(fft(Fxy,L,1).*vecSinc,L,1);
    if(L > M)
        z0 = z0(1:(L/M):end,:);
    end
    z0 = z0(1:floor(FMTSE.N/2)+1,:); % sayan added floor
...
for i=1:size(x,2)
    ...
    N=lnsd(x{i})+1;
    mmat=fft(psimat.*vrep(x{i},size(psimat,N),N),[],dim); %calculating the FFT of signal times PSI. Then take only positives.
    mmat(isnan(mmat))=nan; %This is to prevent imaginary NaNs
    varargout{i+1}=vindex(mmat,index,dim); %taking only positive spectra %sayan commented 
    clear mmat;

    %xmat=vrep(x{i},size(psimat,N),N);
    %mmat=fft(psimat.*xmat,[],dim); %calculating the FFT of signal times PSI. Then take only positives.
    %mmat=fft(bsxfun(@times,FMTSE.S,xmat),[],dim); %sayan
    %varargout{i+1}=mmat(index,:,:);
    %varargout{i+1}=mmat; 
end
varargout{4} = memory_watch();
