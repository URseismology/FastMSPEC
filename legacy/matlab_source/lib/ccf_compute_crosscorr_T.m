function [saved_ccf_path] = ccf_compute_crosscorr_T(ccf_data_file,path_vars,parameters,stacktype,filttype)
    ...
    elseif filttype.CodaProcessing
        fftS1Z = time_spec_normalization_codacorr(S1_data_mat,parameters);
        fftS2Z = time_spec_normalization_codacorr(S2_data_mat,parameters);

    elseif filttype.IsMspec
        saved_ccf_path = ccf_compute_crosscorr_mtc_T(ccf_data_file,path_vars,parameters,stacktype,filttype);
        return
    ...
    saved_ccf_path = ccf_save_computed_ccf_T(coh_trace,coh_num_size,coh_num,path_vars,parameters,stapairsinfo,stacktype);
