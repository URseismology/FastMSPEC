...
%Multitaper
IsMultiTaper = 0;
NW = 11; % Time-Bandwidth Product
K_taps = 21; %Number of Tapers
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

A = readtable('/scratch/tolugboj_lab/Sayan_Swar_WS/PythonEnv/Python_Notebooks/PRJ_SPAC/data/test/metadata/orientation.csv');
slist = A{:,'Var1'};
orientations = A{:,'Var2'};
...
for ipath = 1:length(PATHS)
    ccfZ_path = [PATHS{ipath},'ccf',strNAMEcompT,'/'];
    if ~exist(ccfZ_path)
        mkdir(ccfZ_path);
    end
end
...
for ista1=1:nsta
    ...
    seisZ_path = [seis_winlength_path,strNAMEcompT(1),'/'];
    ...
    for ista2=1:nsta
        ...
        for ifil = 1:length(list1)
            ...
            file1ch1 = strrep(file1cZ,'Z.sac','N.sac');
            file1ch2 = strrep(file1cZ,'Z.sac','E.sac');
            ...
            clear data1cZ data2cZ data1ch1 data2ch1 data1ch2 data2ch2
            ihday = ihday +1;
            ...
            data1cZ =  [datadir,sta1,'/',year,'/',data1cZ.name]; %this code is looking for year folder. might not be needed
            data2cZ =  [datadir,sta2,'/',year,'/',data2cZ.name]; %this code is looking for year folder. might not be needed

            data1ch1 = dir([datadir,sta1,'/',year,'/',sta1,'.',hdayid,'.*',strSACcomp1,'.sac']);
            data2ch1 = dir([datadir,sta2,'/',year,'/',sta2,'.',hdayid,'.*',strSACcomp1,'.sac']);

            data1ch1 =  [datadir,sta1,'/',year,'/',data1ch1.name]; %this code is looking for year folder. might not be needed
            data2ch1 =  [datadir,sta2,'/',year,'/',data2ch1.name]; %this code is looking for year folder. might not be needed

            data1ch2 = dir([datadir,sta1,'/',year,'/',sta1,'.',hdayid,'.*',strSACcomp2,'.sac']);
            data2ch2 = dir([datadir,sta2,'/',year,'/',sta2,'.',hdayid,'.*',strSACcomp2,'.sac']);

            data1ch2 =  [datadir,sta1,'/',year,'/',data1ch2.name]; %this code is looking for year folder. might not be needed
            data2ch2 =  [datadir,sta2,'/',year,'/',data2ch2.name]; %this code is looking for year folder. might not be needed

            %------------------- TEST IF DATA EXIST------------------------
            [S1Zt,S1Zraw,S1,S1Ztstart] = load_sac(data1cZ);
            [S2Zt,S2Zraw,S2,S2Ztstart] = load_sac(data2cZ);

            [S1ch1t,S1ch1raw,S1ch1,S1ch1tstart] = load_sac(data1ch1);
            [S2ch1t,S2ch1raw,S2ch1,S2ch1tstart] = load_sac(data2ch1);

            [S1ch2t,S1ch2raw,S1ch2,S1ch2tstart] = load_sac(data1ch2);
            [S2ch2t,S2ch2raw,S2ch2,S2ch2tstart] = load_sac(data2ch2);

                        
            % Check that sample rates are the same
            if S1.DELTA ~= S2.DELTA || S1ch1.DELTA ~= S2ch1.DELTA || S1ch2.DELTA ~= S2ch2.DELTA
                error('S1 and S2 sample rates don''t match!');
            end
            
            % Make sure all times (of both waveforms) are relative to same
            % reference point. That is both waveforms start at the same
            % time.
            starttime = S1Ztstart;
            S1Zt = S1Zt + seconds(S1Ztstart-starttime);
            S2Zt = S2Zt + seconds(S2Ztstart-starttime);

            S1ch1t = S1ch1t + seconds(S1ch1tstart-starttime);
            S2Zt = S2Zt + seconds(S2ch1tstart-starttime);

            S1ch2t = S1ch2t + seconds(S1ch2tstart-starttime);
            S2ch2t = S2ch2t + seconds(S2ch2tstart-starttime);
            
            % Ensure that files have same start time to within 1 sample
            if abs(seconds(S1Ztstart-S2Ztstart)) > S1.DELTA || abs(seconds(S1ch1tstart-S2ch1tstart)) > S1ch1.DELTA || abs(seconds(S1ch2tstart-S2ch2tstart)) > S1ch2.DELTA
        ...
        zeroind1 = find(S1ch1raw == 0);
        zeroind2 = find(S2ch1raw == 0);
        if length(zeroind1) == length(S1ch1raw) || length(zeroind2) == length(S2ch1raw)
            disp('All zeros!');
            continue
        end

        if (length(S1ch1t)*length(S1ch2t)*length(S1Zt)*length(S2ch1t)*length(S2ch2t)*length(S2Zt)==0)
            disp(['no data for ! station ',sta2]);
            continue
        end
        ...
        if length(S1ch1raw) < 30000 || length(S1ch2raw) < 30000 
            disp(['Sta1 ',sta1,' : ',num2str(length(S1Zraw)),' is too short!'])
            continue
        elseif length(S2ch1raw) < 30000 || length(S2ch2raw) < 30000 
        ...
        if(~exist('lat2','var'));
            S1_ch1 = readsac(data1ch1);
            S2_ch1 = readsac(data2ch1);
    
            lat1=S1_ch1.STLA;
            lon1=S1_ch1.STLO;
            dep1=S1_ch1.STEL; % depth is negative for OBS and positive for land stations


            lat2=S2_ch1.STLA;
            lon2=S2_ch1.STLO;
            dep2=S2_ch1.STEL; % depth is negative for OBS and positive for land stations
...
%             tic
            for iwin = 1:nwin %parfor
%			clear tcut S1R S2R S1T S2T S1Z S2Z fftS1R fftS2R fftS1T fftS2T fftS1Z fftS2Z
                disp([num2str(iwin),'-',hdayid]);
			    % cut in time
                if hour_length == 24
                    pts_begin = Nstart;
                    pts_end = length(S1ch1raw)-Nstart;
                else
                    pts_begin = win_length*.5*(iwin-1)+1+Nstart;
                    pts_end = pts_begin+win_length;
                end
    
                if pts_begin > length(S1ch1raw) || pts_begin > length(S2ch1raw) || pts_end > length(S1ch1raw) || pts_end > length(S2ch1raw)
				    % disp('(Z) Points greater than the data... fixing window');
				    pts_begin = length(S1ch1raw)-win_length-Nstart;
                    pts_end = pts_begin+win_length;
                elseif pts_begin > length(S1ch2raw) || pts_begin > length(S2ch2raw) || pts_end > length(S1ch2raw) || pts_end > length(S2ch2raw)
				    % disp('(Z) Points greater than the data... fixing window');
				    pts_begin = length(S1ch2raw)-win_length-Nstart;
                    pts_end = pts_begin+win_length;
                ...
                S1ch1=interp1(S1ch1t,S1ch1raw,tcut);
                S1ch1(isnan(S1ch1))=0;
                S2ch1=interp1(S2ch1t,S2ch1raw,tcut);
                S2ch1(isnan(S2ch1))=0;

                S1ch2=interp1(S1ch2t,S1ch2raw,tcut);
                S1ch2(isnan(S1ch2))=0;
                S2ch2=interp1(S2ch2t,S2ch2raw,tcut);
                S2ch2(isnan(S2ch2))=0;
                ...
                if IsTaper
                    S1Z = cos_taper(S1Z);
                    S2Z = cos_taper(S2Z);
                    S1ch1 =  cos_taper(S1ch1);
                    S1ch2 =  cos_taper(S1ch2);
                    S2ch1 =  cos_taper(S2ch1);
                    S2ch2 =  cos_taper(S2ch2);
                end
                ...
                if IsPrefilter
                    ...
                    S1ch1 =  FiltFiltM(bb,aa,S1ch1);
                    S2ch1 =  FiltFiltM(bb,aa,S2ch1);

                    S1ch2 =  FiltFiltM(bb,aa,S1ch2);
                    S2ch2 =  FiltFiltM(bb,aa,S2ch2);
                end

                Ista = strcmp([net1, '-', sta1],slist);
                S1phi = orientations(Ista); % angle between H1 and N (CW from north)
                Ista = strcmp([net2, '-', sta2],slist);
                S2phi = orientations(Ista); % angle between H1 and N (CW from north)         
                if S1phi > 5  % correct the orientation if the error is greater than 5 degrees
                    [S1R,S1T] = rotate_vector(S1ch1,S1ch2,S1az-S1phi);
                else
                    [S1R,S1T] = rotate_vector(S1ch1,S1ch2,S1az);
                end
                if S2phi > 5
                    [S2R,S2T] = rotate_vector(S2ch1,S2ch2,S2az-S2phi+180);
                else
                    [S2R,S2T] = rotate_vector(S2ch1,S2ch2,S2az+180);
                end


                %-------------------- Transverse Component (Love) --------------
                if IsFTN
                ...
                elseif IsMultiTaper
                    ...
                    if tcut_last_len ~= length(tcut)
                        [PSI,LAMBDA] = sleptap(length(tcut),NW,K_taps);
                        tcut_last_len = length(tcut);
                    end                    

                    tapered_S1T = (S1T.' .* PSI).';
                    tapered_S2T = (S2T.' .* PSI).';
                    
                    fftS1T = mean(fft(tapered_S1T,[],2),1);
                    fftS2T = mean(fft(tapered_S2T,[],2),1);
                ...
                coh_trace = fftS2T .* conj(fftS1T); %2-1
                ...
                try
                    ...
                    disp(['Less data for ',sta2,' on day ',hdayid,'... skipping'])
                    continue
                end
    ...
    %             toc
                ...
                if IsOutputSinglestack % save individual xcor
                    ccfZ_singlestack_path = [ccf_singlestack_path,'ccf',strNAMEcompT,'/'];
...
%             toc
            ...
            if IsOutputDaystack
                ...
                ccfT_daystack_path = [ccf_daystack_path,'ccf',strNAMEcompT,'/'];
                clear coh_sum
            ...
            if IsOutputMonthstack
                ...
                if month_counter == 30
                    imonth = imonth + 1;
                    ccfT_monthstack_path = [ccf_monthstack_path,'ccf',strNAMEcompT,'/'];
                    clear coh_sum
...
%                 set(gcf,'position',[400 400 600 300]);
                ...
                title(sprintf('%s %s coherency %s ,station distance: %f km',sta1,sta2,strNAMEcompT(1),dist));
%                 xlim([0.01 1/(dt*2.5)])
                xlim([0.01 0.5])
                %xlim([0.04 0.16])
                xlabel('Frequency')
...
%                 xlim([0.01 1/(dt*2.5)])
                ...
                print(f101,'-dpsc',[fig_winlength_path,sta1,'_',sta2,'_',strNAMEcompT,'.ps']);
                %pause;
            end
            if IsOutputFullstack
                ccfT_fullstack_path = [ccf_fullstack_path,'ccf',strNAMEcompT,'/'];
                clear coh_sum
                coh_sum = coh_sumT;
