...
IsDetrend = 0; % detrend the data
IsTaper = 0; % Apply cosine taper to data chunks
...
%			clear tcut S1R S2R S1T S2T S1Z S2Z fftS1R fftS2R fftS1T fftS2T fftS1Z fftS2Z
                ...
                elseif IsMultiTaper
                   ...
                   %[F,SXX,SYY,coh_trace] = mspec(S2Z,S1Z,PSI);
                    

                    tapered_S1Z = (S1Z.' .* PSI).';
                    tapered_S2Z = (S2Z.' .* PSI).';
                    
                    fftS1Z = mean(fft(tapered_S1Z,[],2),1);
                    fftS2Z = mean(fft(tapered_S2Z,[],2),1);
...
%                     ind = find(faxis>0);
%                     %plot(faxis(ind),real(fftS1Z(ind)));
%                     plot(real(fftS1Z));
% 
%                     figure(902)
%                     temp=fft(S1Z);
...
%                     faxis = [0:(T-mod(T-1,2))/2 , -(T-mod(T,2))/2:-1]/dt/T;
%                     ind = find(faxis>0);
%                     %plot(faxis(ind),real(temp(ind)));
%                     plot(real(temp));
%                     disp('sectional plot')

                    %scaling_factor_S1Z = rms(S1Z) ./ rms(tapered_S1Z,2);
                    %scaling_factor_S2Z = rms(S2Z) ./ rms(tapered_S2Z,2);
                    %rescaled_tapered_S1Z = tapered_S1Z .* scaling_factor_S1Z;
                    %rescaled_tapered_S2Z = tapered_S1Z .* scaling_factor_S2Z;                    
                    %fftS1Z = sum(fft(rescaled_tapered_S1Z),1) .* (1/K_taps);  %mean(fft(tapered_S1Z),1);
                    %fftS2Z = sum(fft(rescaled_tapered_S2Z),1) .* (1/K_taps);  %mean(fft(tapered_S2Z),1);
