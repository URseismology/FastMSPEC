...
| J | **Global validation program** (2,000 stations) | **blocked** on the gates in the readiness document | Love-wave (transverse) smoke test; timing audit on a station sample; window/bandwidth design per distance (Notebook 05); taper/overlap padding; reference curve outside Africa; pilot of 50–100 stations | [`global_validation_readiness.md`](global_validation_readiness.md) |
| K | **Acquisition-pipeline smoke test** (WaveNet-EpicAI, XD.MTAN–XD.RUNG) — *adjacent project, separate folder* | **done and pushed** 2026-09-25: `URseismology/wavenet-epicAI` branch `add-september-ncf-pipeline`, commit `789a49c`, folder `docs/ncf_pipeline_stages/xd_mtan_rung_smoke_test/` (report + PDF, verified patches, tests, evidence). Fixed pipeline: 0 refused days, benchmark corr +0.83 / 12 of 12 zero crossings / −0.6 % versus +0.65 / 8 of 12 / −2.3 % as found | Love-wave (transverse) smoke test on the same pair; timing audit on a station sample; window-level masking of periodic calibration events; authors decide adoption of the patches | local `~/claude-sandbox/projects/wavenet_xd_pair_test/STATE.md` (not in this repository) |
...
* **Data-quality facts from the test:** corrupt raw days recur on a 5-day cadence at ≈ 17:00 UTC on both stations (scheduled calibration or mass re-centering), so QC should mask windows, not whole days; XD.RUNG BHN is persistently heavy-tailed and needs spike screening before Love-wave use; 5 of 318 station-days were dropped by the original pipeline (14 of 954 channel-days).
